...
#tool_offset = emccanon.GET_EXTERNAL_OFFSETS()
#tool_offset.z = 25.4321

#print tool_offset

#emccanon.SET_TOOL_TABLE_ENTRY(45,1,tool_offset,.25,0,0,0)
...
ATC_NUM_POCKETS = int(INI.find("ATC", 'POCKETS') or 0)
...
def _find_assigned_pocket(tnum):
    """Returns the pre-assigned pocket for the tool for non-random ATCs"""
    tool_table = _read_tool_table()
    print "Find tool pocket", tnum, tool_table[tnum].get('P', -1.00)
    return tool_table[tnum].get('P', -1)
...
def _atc_list_model(force_read=False):
    ...
    for tnum, tdata in _read_tool_table(force_read).items():
        pnum = tdata['P']
...
def change_prolog(self, **words):
    try:
        ...
        self.params["stored_tool"] = self.current_tool
